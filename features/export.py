    ...
    try:
        ...
        for page_idx, page in enumerate(editor.pages):
            ...
            for box_data in page.get("text_boxes", []):
                ...
                # 检查是否为竖排文字
                is_vertical = box_data.get("vertical", False)
                if is_vertical:
                    # 使用 XML 设置竖排方向 (eaVert = East Asian Vertical)
                    try:
                        tf._bodyPr.attrib['vert'] = 'eaVert'
                    except Exception:
                        pass
                ...
                for box_data in page.get("text_boxes", []):
                    ...
                    try:
                        ...
                        if is_vertical:
                            # 竖排文字：逐字垂直绘制
                            try:
                                sample_char = box_data["text"][0] if box_data["text"] else "中"
                                ...
                                # 垂直居中整体文字块
                                total_text_h = char_h * len(box_data["text"])
                                start_y = y + (h - total_text_h) // 2
                                
                                for i, char in enumerate(box_data["text"]):
                                    char_y = start_y + i * char_h
                                    draw.text((text_x, char_y), char, font=font, fill=(r, g, b))
                            except Exception:
                                # 回退：简单逐字绘制
                                for i, char in enumerate(box_data["text"]):
                                    draw.text((x + 3, y + 3 + i * pixel_font_size), char, font=font, fill=(r, g, b))
                        else:
                            # 横排文字：原有逻辑
                            try:
